...
bin/run_caller.sh --caller=codex --conf-file=conf/caller.yaml --caller-path=R/run_CODEX.R --coverage-file=coverage.tsv --sample-names-file=$(Rscript R/test/get_toy_data.R sampname) --bed-file=$(Rscript R/test/get_toy_data.R bedFile)
