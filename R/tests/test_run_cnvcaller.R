...
context("Testing read_coverage_table function")
 ...
 test_that("basic test for reading coverage table from database",{
   coverage_table <- dbGetQuery(conn_psql, "select * from test_coverage")
   expect_equal(nrow(coverage_table), 2)
   expect_equal(coverage_table[1,"chr"] != coverage_table[2,"chr"], TRUE)
   read_coverage_table_result <- read_coverage_table("test_coverage", conn_psql, coverage_table[1,"chr"])
   ...
   expect_equal(read_coverage_table_result[1,"read_count"], coverage_table[1,"read_count"])
   read_coverage_table_result <- read_coverage_table("test_coverage", conn_psql, coverage_table[2,"chr"])
   ...
   expect_equal(read_coverage_table_result[1,"read_count"], coverage_table[2,"read_count"])
 })
...
test_that("basic test for saving codex calls to database",{
  dbSendUpdate(conn_psql, "delete from test_calls where sample_name='cnv_test_sample'")
  calls <- matrix(nrow=2, ncol=12)
  colnames(calls) <- c('sample_name', 'chr', 'cnv', 'st_bp', 'ed_bp', 'st_exon', 'ed_exon', 'raw_cov', 'norm_cov', 'copy_no', 'codex_lratio', 'codex_mBIC')
  calls[1,] <- c('cnv_test_sample', 1, 'dup', 1000, 2000, 7, 8, 230, 130, 3, 25.111, 23.222)
  calls[2,] <- c('cnv_test_sample', 2, 'del', 2000, 4000, 14, 16, 460, 260, 6, 50.222, 46.444)
  parameters_id <- 1
  scenario_id <- 1
  calls_table <- 'test_calls'
  caller <- 'codex'
  save_calls(calls, calls_table, caller, scenario_id, parameters_id, conn_psql)
  ...
  expect_equal(saved_calls[1,"codex_lratio"], '25.111')
  expect_equal(saved_calls[1,"codex_mbic"], '23.222')
  expect_equal(saved_calls[1,"exomedepth_bf"], '0.00')
  ...
  expect_equal(saved_calls[2,"codex_lratio"], '50.222')
  expect_equal(saved_calls[2,"codex_mbic"], '46.444')
  expect_equal(saved_calls[2,"exomedepth_bf"], '0.00')
  dbSendUpdate(conn_psql, "delete from test_calls where sample_name='cnv_test_sample'")
})

test_that("basic test for saving exomedepth calls to database",{
  dbSendUpdate(conn_psql, "delete from test_calls where sample_name='cnv_test_sample'")
  calls <- matrix(nrow=2, ncol=11)
  colnames(calls) <- c('sample_name', 'chr', 'cnv', 'st_bp', 'ed_bp', 'st_exon', 'ed_exon', 'raw_cov', 'norm_cov', 'copy_no', 'exomedepth_BF')
  calls[1,] <- c('cnv_test_sample', 1, 'dup', 1000, 2000, 7, 8, 230, 130, 3, 23.222)
  calls[2,] <- c('cnv_test_sample', 2, 'del', 2000, 4000, 14, 16, 460, 260, 6, 46.444)
  parameters_id <- 1
  scenario_id <- 1
  calls_table <- 'test_calls'
  caller <- 'exomedepth'
  ...
  expect_equal(saved_calls[1,"norm_cov"], '130')
  expect_equal(saved_calls[1,"copy_no"], '3')
  expect_equal(saved_calls[1,"codex_lratio"], '0.00')
  expect_equal(saved_calls[1,"codex_mbic"], '0.00')
  expect_equal(saved_calls[1,"exomedepth_bf"], '23.222')
  expect_equal(saved_calls[2,"parameters_id"], parameters_id)
  expect_equal(saved_calls[2,"sample_name"], 'cnv_test_sample')
  expect_equal(saved_calls[2,"chr"], '2')
  expect_equal(saved_calls[2,"cnv"], 'del')
  ...
  expect_equal(saved_calls[2,"norm_cov"], '260')
  expect_equal(saved_calls[2,"copy_no"], '6')
  expect_equal(saved_calls[2,"codex_lratio"], '0.00')
  expect_equal(saved_calls[2,"codex_mbic"], '0.00')
  expect_equal(saved_calls[2,"exomedepth_bf"], '46.444')
  dbSendUpdate(conn_psql, "delete from test_calls where sample_name='cnv_test_sample'")
})

test_that("basic test for saving canoes calls to database",{
  dbSendUpdate(conn_psql, "delete from test_calls where sample_name='cnv_test_sample'")
  calls <- matrix(nrow=2, ncol=8)
  colnames(calls) <- c('sample_name', 'chr', 'cnv', 'st_bp', 'ed_bp', 'st_exon', 'ed_exon', 'copy_no')
  calls[1,] <- c('cnv_test_sample', 1, 'dup', 1000, 2000, 7, 8, 3)
  calls[2,] <- c('cnv_test_sample', 2, 'del', 2000, 4000, 14, 16, 6)
  parameters_id <- 1
  scenario_id <- 1
  calls_table <- 'test_calls'
  caller <- 'canoes'
  ...
  expect_equal(saved_calls[1,"st_exon"], '7')
  expect_equal(saved_calls[1,"ed_exon"], '8')
  expect_equal(saved_calls[1,"raw_cov"], '0')
  expect_equal(saved_calls[1,"norm_cov"], '0')
  expect_equal(saved_calls[1,"copy_no"], '3')
  expect_equal(saved_calls[1,"codex_lratio"], '0.00')
  expect_equal(saved_calls[1,"codex_mbic"], '0.00')
  expect_equal(saved_calls[1,"exomedepth_bf"], '0.00')
  ...
  expect_equal(saved_calls[2,"st_exon"], '14')
  expect_equal(saved_calls[2,"ed_exon"], '16')
  expect_equal(saved_calls[2,"raw_cov"], '0')
  expect_equal(saved_calls[2,"norm_cov"], '0')
  expect_equal(saved_calls[2,"copy_no"], '6')
  expect_equal(saved_calls[2,"codex_lratio"], '0.00')
  expect_equal(saved_calls[2,"codex_mbic"], '0.00')
  expect_equal(saved_calls[2,"exomedepth_bf"], '0.00')
